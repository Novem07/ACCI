...
export default function usePayments({ page, pageSize, query, status }) {
  const [state, setState] = useState({ items: [], page, pageSize, totalItems: 0, totalPages: 0, loading: true, error: null });
  useEffect(() => {
    const controller = new AbortController();
    setState((current) => ({ ...current, loading: true, error: null }));
    listPayments({ page, pageSize, query, status }, { signal: controller.signal }).then((result) => setState({ ...result, loading: false, error: null })).catch((error) => {
